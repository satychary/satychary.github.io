...
var wantToolbar = 0;   // 0 if toolbar isn't wanted
var mouseClickEnabled = false;  // enables left click for next slide
...
strings_pl[helpText] =
"Zmieniaj slajdy klikaj?c mysz?, naciskaj?c spacj?, strza?ki lewo/prawo" +
"lub PgUp / PgDn. U?yj klawiszy S i B, aby zmieni? rozmiar czczionki.";
...
"Page Up billenty?ket. Az S és a B billenty?kkel változtathatja " +
"a szöveg méretét.";
...
function startup()
{
	...
	if (slides.length > 0)
	{
		...
		slide.style.position = "relative"; // "absolute";
    ...
    // mod to help viewers using mobile browsers..
	// if(Math.random()<0.25)
                // Aug '16: DON'T start w/ 'all'
	//////////toggleView(); // always start w/ 'all' :)
}// startup
...
// Saty's version (orig. renamed showToolbar_orig)
function showToolbar_Saty()
{
...
function previousSlide(incremental)
{
	if (!viewAll)
	{
		...
		// Saty's add'n: // carousel backwards :)
		else if (slidenum == 0)
		{
...
function nextSlide(incremental)
{
	if (!viewAll)
	{
		...
		// Saty's addition - carousel forwards
		else if ((!incremental || lastShown == null) && (slidenum ==slides.length - 1))
		{
			slide = slides[slidenum];
			hideSlide(slide);
...
function showSlide(slide)
{
	...
	////////// the foll. was causing upwards scrolling 
	////////// to SNAP to the very top, in Chrome -
	////////// VERY annoying!
	////////// window.scrollTo(0,0);
...
function showAllSlides()
{
	...
	for (var i = 0; i < slides.length; ++i)
	{
		...
		slide.style.borderTopStyle = "none"; // was "solid"; - but we don't need these separators!
		...
		setVisibilityAllIncremental("visible");
		showSlide(slide); // add each slide to the list (view) of all slides
	}
...
function patchAnchors()
{
	...
	for (var i = 0; i < anchors.length; ++i)
	{
		...
		//////////anchors[i].onclick = clickedAnchor;
                // now I can SIMPLY click on ANY link to have it open in a new tab!!!!!
	}
}
...
function showSlideNumber()
{
	...
	(slidenum + 1) + "/" + slides.length;
                // saty's mod
                slideNumElement.innerHTML = (slidenum + 1) + "/" + slides.length;
}
...
function addToolbar()
{
	...
	if (ns_pos) // a reasonably behaved browser
	{
		...
		// Saty's mod - works!
		//slideCounter = document.createElement("div"); // need this!
                slideCounter = document.getElementById("slideNum"); // need this!
		slideCounter.innerHTML = "slide".localize() + " n/m";
                slideCounter.innerHTML = "n/m";
                //slideCounter.style.height="0.5em";
                //slideCounter.style.font = "italic bold 20px arial,serif";
                slideCounter.style.fontSize = "14px";
                slideCounter.style.font = "regular normal 14px Helvetica,sans-serif";
                slideCounter.style.fontFamily="Arial,Helvetica Neue,Helvetica,sans-serif"; 
                slideCounter.style.fontSize="14px";
                slideCounter.style.fontStyle="normal";
                slideCounter.style.color = "#9696B7";
		...
		eos.innerHTML = "* ";
		// Saty's mod
                // no need for this - I can look for n/m to know it's the end :)
	...
	else // IE so need to work around its poor CSS support
	{
		...
		help.setAttribute("title", helpText.localize());
		// help.innerHTML = "help?".localize();
		// Saty's mod - leave out the string
		...
		//slideCounter.style.height = "0.6em";
		...
		slideCounter.innerHTML = "slide".localize() + " n/m";
                slideCounter.innerHTML = "n/m";
                //slideCounter.style.font = "italic bold 20px arial,serif";
                slideCounter.style.fontSize = "6px";
                alert(slideCounter.innerHTML);
...
function showTableOfContents()
{
	if (toc)
	{
		if (toc.style.visibility != "visible")
		{
			...
			toc.style.background = "rgb(100,100,100)"; // saty
			toc.focus();
...
function tableOfContents()
{
	...
	heading.innerHTML = "TOC".localize();
	...
	toc.style.background = "rgb(180,180,200)";
	toc.style.borderStyle = "solid";
	toc.style.borderWidth = "1px";
	toc.style.fontSize = "12px"; // "90%";
...
// all done, so do toggleView() to start off in ss mode
// toggleView();
